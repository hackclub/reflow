    ...
    def run(self) -> None:
        for file in self.deps.files_list:
            if file.endswith(("kicad_pcb")):
                ...
                try:
                    subprocess.run(f"kicad-cli pcb export gerbers -o sanity_output/gerbers/ --format gerberx2 ../{file}", shell=True) # we can discard the output of this
                    subprocess.run(f"kicad-cli pcb export drill -o saniity_output/gerbers/ ../{file}") # and this
